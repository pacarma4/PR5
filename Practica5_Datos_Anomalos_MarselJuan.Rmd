...
hampel <- function(x){
  ...
  lim_sup <- mediana + 3*MADM
  lim_inf <- mediana - 3*MADM
...
percentiles <- function(x){
  percentil <- quantile(x, prob= c(0.05,0.95), na.rm =TRUE)
  lim_sup <- percentil[2]
  lim_inf <- percentil[1]
...
clean_iris <-clean_iris %>%
  mutate( across(where(is.numeric), list(Sigma = sigma3, hampel=hampel, boxplot=boxplot, percentiles = percentiles)))
...
clean_iris %>%
  summarise(across(contains("_"),~sum(!.,na.rm = TRUE)))
```
...
clean_iris %>%
  select(Petal.Length,starts_with("Petal.Length")) %>%
  pivot_longer(cols = -Petal.Length, names_to = "Método", values_to = "No_es_Outlier") %>%
  ggplot(aes(x=Método, y = Petal.Length)) + 
  geom_point(aes(color=No_es_Outlier), position = "jitter")
```
...
dirty_iris %>% 
  mutate(Petal.Length1 = ifelse(is.na(Petal.Length),mean(Petal.Length, na.rm=TRUE),Petal.Length))
```
...
dirty_iris %>%
  group_by(Species) %>%
  mutate(Petal.Length2 =ifelse(is.na(Petal.Length),mean(Petal.Length, na.rm=TRUE),Petal.Length))
